...
namespace makVrv
{
   ...
   static std::string accessoryName="MyAccessoryHla13";
      ...
      void MyAccessory::install(DtDriver* driver)
      {
         ...
         std::cout << "[ INFO ] Create Trainee State Fom Mapper " << std::endl;
         //TITS::traineeFom(vrlCon->exerciseConn(), false);

         traineeFom = new TITS::TraineeFom();
         traineeFom->traineeFom(vrlCon->exerciseConn(), false);
      ...
      void MyAccessory::uninstall(DtDriver* baseConn)
      {
          ...
          std::cout << "[ INFO ] Unload Trainee State Fom Mapper " << std::endl;
          traineeFom->untraineeFom(vrlCon->exerciseConn());
      ...
      /*
      void MyAccessory::setHelmetPatchState(byte helmet)
      {
          traineeFom->setHelmetPatchState(helmet);
      }

      void MyAccessory::setBodyPatchState(byte body)
      {
          traineeFom->setBodyPatchState(body);
      }

      void MyAccessory::setHandPatchState(byte hand)
      {
          traineeFom->setHandPatchState(hand);
      }
      */
   }
}
